...
export const AnnotationHistory = injector(
  observer(({ annotationStore, selected, createdBy, selectedHistory, history }) => {
    return (
...
const HistoryItem = observer(
  ({ entity, user, date, extra, acceptedState, selected = false, selectable = true, onClick }) => {
      ...
      <Block name="history-item" mod={{ selected, disabled: !selectable }} onClick={onClick}>
        <Space spread>
          ...
          <Space size="small">
            ...
            {date ? (
              <Elem name="date">{formatDistanceToNow(new Date(date), { addSuffix: true })}</Elem>
            ) : extra ? (
